...
drop_columns = ["ls", "st", "rs", "lw", "lf", "cf", "rf", "rw", "lam", "cam", "ram", "lm", "lcm",
    "cm", "rcm", "rm", "lwb", "ldm", "cdm", "rdm", "rwb", "lb", "lcb", "cb", "rcb", "rb"]

df21 = F21.drop(columns=drop_columns)

correlation = df21.corr()
target_correlation = correlation['overall'].abs().sort_values(ascending=False)
# Filtering features with correlation greater than 0.2
important_features = target_correlation[target_correlation > 0.2]

# Print the filtered features
# print(important_features.drop(columns='overall'))

# List of features keept
selected_features = [
    "overall",
    "value_eur",
    "release_clause_eur",
...
selected_features_df21 = df21[selected_features]
selected_features_df21

y=selected_features_df21['overall']
X=selected_features_df21.drop('overall',axis=1)

# Save the 'F21' DataFrame to a CSV file without including the index

selected_features_df21.to_csv("F21processed.csv", index=False)
...
"""Exporting the model to use in the source file"""
...
"""Checking correlation values of columns"""
...
df22 = F22.drop(columns=drop_columns)
...
# Create a new DataFrame with the selected features
selected_features_df22 = df22[selected_features]
selected_features_df22

column_names_22 = selected_features_df22.drop('overall',axis=1).columns
X_22 = selected_features_df22[column_names_22]
y_22 = selected_features_df22['overall']
scaler.fit(X_22)
...
accuracy_percentage_em = 100 * (1 - em_mae / (max_target - min_target))
accuracy_percentage_em

max_target = max(y_22)
min_target = min(y_22)

accuracy_percentage_rf = 100 * (1 - rf_mae / (max_target - min_target))
accuracy_percentage_rf
...
best_rf_model.score(X_test_scaled, y_test)

ensemble.score(X_test_scaled, y_test)

print(selected_features_df22.columns)
print(len(selected_features_df22.columns))